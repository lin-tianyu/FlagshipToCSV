...
from nnunetv2.utilities.utils import create_lists_from_splitted_dataset_folder, create_lists_from_splitted_dataset_folder_with_csv
...
class nnUNetPredictor(object):
    ...
    def _manage_input_and_output_lists(self, list_of_lists_or_source_folder: Union[str, List[List[str]]], csv_path: str, output_csv_path: str, 
        ...
        if isinstance(list_of_lists_or_source_folder, str):
            list_of_lists_or_source_folder = create_lists_from_splitted_dataset_folder_with_csv(list_of_lists_or_source_folder, csv_path, output_csv_path,
    ...
    def predict_from_files(self,
                           list_of_lists_or_source_folder: Union[str, List[List[str]]], csv_path: str, output_csv_path: str,
        ...
        list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files = \
            self._manage_input_and_output_lists(list_of_lists_or_source_folder, csv_path, output_csv_path,
        ...
        return self.predict_from_data_iterator(data_iterator, save_probabilities, num_processes_segmentation_export, output_csv_path)
    ...
    def predict_from_data_iterator(self,
                                   data_iterator,
                                   save_probabilities: bool = False,
                                   num_processes_segmentation_export: int = default_num_processes, output_csv_path: str = None):
        """
                    ...
                    r.append(
                        export_pool.starmap_async(
                            ...
                            ((prediction, properties, self.configuration_manager, self.plans_manager,
                              self.dataset_json, ofile, save_probabilities, output_csv_path),)
                        )
                    )
                else:
                    # convert_predicted_logits_to_segmentation_with_correct_shape(
...
def predict_entry_point_modelfolder():
    ...
    # NOTE: create a new csv and rename it
    # print(addon_column_names)
    if not os.path.exists(args.output_csv):
        os.makedirs(os.path.dirname(args.output_csv), exist_ok=True)
        addon_column_names = pd.read_csv("csv_header(in).csv").columns.tolist()[1:] # the first one is `bdmap_id`, already exists
        copy_df = pd.read_csv(args.input_csv)
        copy_df.rename(columns={"BDMAP ID": "bdmap_id"}, inplace=True)
        copy_df.drop(columns=["Original ID"], inplace=True)
        for column_name in addon_column_names:
            # if any([x in column_name for x in ["voxel_size", "volume_size"]]):
            copy_df[column_name] = None
        copy_df.to_csv(args.output_csv, index=False)
    else:
        pass

    predictor.predict_from_files(args.i, args.input_csv, args.output_csv, args.o, save_probabilities=args.save_probabilities,
